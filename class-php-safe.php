...
class PHP_SAFE extends PHP_Parser {
    ...
    function parse_user_defined_function_vulnerability($file_name, $function_name, $block_start_index, $block_end_index, $target_function_name) {
        ...
        for ($i = 0, $count = count($this->files_functions); $i < $count; $i++) {
            if (0 === strcasecmp($target_function_name, $this->files_functions[$i]['function_name'])) {//note: PHP functions are not case sensitive
                ...
                for ($j = $start_index; $j < $end_index; $j++) {
                    ...
                    if (T_VARIABLE === $this->files->files_tokens[$file_name][$j][0]) {
                        $index = $j;
                        $function_argument_name = $this->get_variable_name($file_name, $index); //$index (the $files_tokens index) is passed by reference
                        ...
                        $prefix = explode('->', $function_parameter_name, 2); //If the variable is an object and it is tainted, then the contents of that variable are also tainted
        ...
        if ($i === $count) {//The function is not an output function, but it is not also defined in the code
            for ($j = $start_index; $j < $end_index; $j++) {
                if (T_VARIABLE === $this->files->files_tokens[$file_name][$j][0]) {
                    $j = $this->parse_variable($file_name, $target_function_name, $j);
                }
        ...
        for ($i = $count - 1; $i >= 0; $i--) {
            if ((0 === strcasecmp($this->parser_variables[$i]['function_name'], $target_function_name) ) && //note: PHP functions are not case sensitive
                ...
                if (EXIST === $this->parser_variables[$i]['exist_destroyed']) {//it the variable was not yet destroyed, then create a variable destroyed
    ...
    function parse_other_function_vulnerability($file_name, $function_name, $block_start_index, $block_end_index, $target_function_name) {
        ...
        if (!is_null($vulnerability_classification)) {// if it is one of the output functions. It is not a user defined function, although it could be a WordPress function
            ...
            for ($i = $block_start_index; $i < $block_end_index; $i++) {
                if (is_array($this->files->files_tokens[$file_name][$i])) {
                    if (($this->is_variable($file_name, $i)) ||
                            ($this->is_property($file_name, $i))) {//if it is a variable
                        //add the variable name, variable used in PHP or outside PHP, input variable, the function name, the file name and the line number and the taint value, variable classification, and the $parserFileTokens array index
                        //create a local variable with the name of the parameter and the contents of the argument
                        $index = $i;
                        $output_variable_name = $this->get_variable_name($file_name, $index); //$index (the $files_tokens index) is passed by reference
                        $prefix = explode('->', $output_variable_name, 2); //If the variable is an object get the name of the object
                        ...
                        if (!is_null($variable_index)) {//if the variable already existed the taint and vulnerability classification should be transferred to the current variable
                            if (TAINTED === $this->parser_variables[$variable_index]['tainted']) {//if the variable is tainted, then we have a vulnerability
...
//                        $this->debug('$start_index ' . $start_index . ' $output_variable_name ' . $output_variable_name . ' $variable_index ' . $variable_index . ' $output_variable_index ' . $output_variable_index . '<br />');
                        $i = $index;
                            ...
                            ($this->is_method($file_name, $i - 2))) {//if it is a function or a method
                        ...
                        foreach (Vulnerable_Input::$INPUT_FUNCTIONS as $key => $value) {//test if it is an input function
                            foreach ($value as $output_function) {
                                if (0 === strcasecmp($output_function, $this->get_variable_function_property_method_name($file_name, $i))) {//note: PHP functions are not case sensitive
                                    $input_variable = INPUT_VARIABLE;
                                    if ($this->is_method($file_name, $i - 2)) {//check to see if it is an object user defined function
                            ...
                            if (!is_null($input_variable)) {//add a new variable, which is the return value of the input function
                                $this->parser_variables[] = array(
                        ...
                        if ($this->is_user_defined_function($file_name, $this->files->files_tokens[$file_name][$i][1])) {//if it is a usere defined function
                            ...
                            if (!is_null($output_variable_index)) {
                                $this->parser_variables[$output_variable_index]['output_variable'] = OUTPUT_VARIABLE;
                                if (TAINTED === $this->parser_variables[$output_variable_index]['tainted']) {//if the variable is tainted, then we have a vulnerability
                                    $this->parser_variables[$output_variable_index]['vulnerability_classification'] = $vulnerability_classification;
                                }
                            }
                        }
                        continue; //continue to the next argument of the function
                    }
                }
            }
        }
    ...
    function parse_return_vulnerability($file_name, $function_name, $block_start_index, $block_end_index) {
        ...
        $variable_index = $this->get_variable_index($function_name, $function_name); // returns null if there is no variable
        if ((!is_null($variable_index)) &&
                (EXIST === $this->parser_variables[$variable_index]['exist_destroyed'] )) {
            if ($expression['tainted'] === TAINTED) {//if the variable is tainted, then we have a vulnerability
    ...
    function parse_expression_vulnerability($file_name, $function_name, $block_start_index, $block_end_index) {
        ...
        for ($i = $block_start_index; $i < $block_end_index; $i++) {
            ...
            if ($this->is_variable($file_name, $i)) {//it is a variable
                $index = $i;
                $variable_name = $this->get_variable_name($file_name, $index); //$index (the $files_tokens index) is passed by reference. When it is a function it returns the vlue 'function'
                $this->parse_variable($file_name, $function_name, $i); //add the variable to the multi-dimensional associative array $parser_variables
                $variable_index = $this->get_variable_index($variable_name, $function_name); // returns null if there is no variable
                ...
                if ('function' === $variable_name) {//if it is an object user defined function
                    ...
                    if (($this->is_method($file_name, $i)) ||
                        ...
                        foreach (Vulnerable_Input::$INPUT_FUNCTIONS as $key => $value) {
                            foreach ($value as $output) {
                                if (0 === strcasecmp($output, $this->get_variable_function_property_method_name($file_name, $i))) {//note: PHP functions are not case sensitive
                                    $input_variable = INPUT_VARIABLE;
                                    break;
                                }
                            }
                        ...
                        if (!is_null($input_variable)) {//update the input_V_variable value of the variable
                ...
                $i = $index;
                    ...
                    ($this->is_method($file_name, $i))) {//a function or method
                ...
                if (!is_null($input_variable)) {//update the input_V_variable value of the variable
                    ...
                    $variable_index = $this->get_variable_index($target_function_name, $function_name); // returns null if there is no variable
                    if (!is_null($variable_index)) {
                        //propagate the tainted attribute
                        if (TAINTED === $this->parser_variables[$variable_index]['tainted']) {
                            $tainted = TAINTED;
                        ...
                        $variable_dependencies_index[] = $variable_index;
                    } else {//it may return null if it is a php function or a function which we have no code. In these cases search for the arguments
                        ...
                        for ($j = $i + 1; $j < $argument_end_index; $j++) {
                            if (is_array($this->files->files_tokens[$file_name][$j])) {
                                if ($this->is_variable($file_name, $j)) {//if it is a variable
                                    ...
                                    //create a local variable with the name of the parameter and the contents of the argument
                                    $jndex = $j;
                                    $output_variable_name = $this->get_variable_name($file_name, $jndex); //$jndex (the $files_tokens index) is passed by reference
                                    ...
                                    if (is_null($output_variable_index)) {
                                        continue;
                                    } else {//if the variable already existed the taint and vulnerability classification should be transferred to the current variable
                                        $variable_filters = $this->get_variable_filters_and_revert_filters($file_name, $j);
                                        if (TAINTED === $this->parser_variables[$output_variable_index]['tainted']) {//if the variable is tainted, then we must check if the function is one of the filtering functions
...
//                        $this->debug('$start_index ' . $start_index . ' $output_variable_name ' . $output_variable_name . ' $variable_index ' . $variable_index . ' $output_variable_index ' . $output_variable_index . '<br />');
    ...
    function parse_as_vulnerability($file_name, $function_name, $block_start_index, $block_end_index, $variable_index) {
        ...
        $new_variable_name = $this->get_variable_name($file_name, $index); //$index (the $files_tokens index) is passed by reference. When it is a function it returns the vlue 'function'
        $this->parse_variable($file_name, $function_name, $block_start_index + 1); //add the variable to the multi-dimensional associative array $parser_variables
        $new_variable_index = $this->get_variable_index($new_variable_name, $function_name); // returns null if there is no variable
...
//        $this->debug('OLA 1 !!! $variable_name ' . $variable_name . '<br />');
        if (!is_null($variable_name) && ($variable_name != 'function')) {
            ...
            foreach (Vulnerable_Input::$INPUT_VARIABLES as $key => $value) {//search for input vulnerable variables
                foreach ($value as $input_array_var) {//search for PHP reserved variables
//                        $this->debug('$input_array_var: ' . $input_array_var . ' - $token[$i][1]: ' . $token[$i][1] . '<br />');
                    if ($this->get_variable_function_property_method_name($file_name, $block_start_index) === $input_array_var) {//if it is a PHP reserved variables
                        $input_variable = INPUT_VARIABLE;
                        $tainted = TAINTED;
//                                $this->debug('$variable_name: '.$variable_name.'<br />');
            ...
            $prefix = explode('->', $variable_name, 2); //If the variable is an object and it is tainted, then the contents of that variable are also tainted
            ...
            if (!is_null($variable_name_index)) {// if it is a variable process it. Otherwise leave this function 
...
//            $this->debug('OLA 2 !!! $variable_name ' . $variable_name . ' $tainted '.$tainted.'<br />');
            if ((is_null($variable_name_index)) ||
                    ...
                    ($this->parser_variables[$variable_name_index]['file_tokens_end_index'] != $block_end_index)))) {// if it is a variable process it. Otherwise leave this function 
                //add the variable name, variable used in PHP or outside PHP, input variable?, the function name, the file name and the line number and the taint value, variable classification, and the $parserFileTokens array index
            ...
            if (($variable_name_index) &&
                    ($this->start_of_php_line($file_name, $block_start_index) > $this->start_of_php_line($file_name, $this->parser_variables[$variable_name_index]['file_tokens_end_index']))) {
            ...
            if (TAINTED === $tainted) {
                ...
                if (((T_OPEN_TAG === $this->files->files_tokens[$file_name][$start_of_php_line_index][0] ) ||
                        ...
                        (T_CLOSE_TAG === $this->files->files_tokens[$file_name][$end_of_php_line_index][0] )) {//if the start and the end of the line are PHP_OPEN_TAG and PH_CLOSE_TAG
                    $is_single_code = true;
            ...
            if (is_array($this->files->files_tokens[$file_name][$i])) {
                if (T_STRING === $this->files->files_tokens[$file_name][$i][0]) {
                    ...
                    foreach (Vulnerable_Filter::$VARIABLE_FILTERS as $key => $value) {
                        foreach ($value as $output) {
                            if (0 === strcasecmp($output, $this->files->files_tokens[$file_name][$i][1])) {//note: PHP functions are not case sensitive
    ...
    function get_variable_name($file_name, &$file_index) {
        if (($this->is_variable($file_name, $file_index)) ||
                ($this->is_property($file_name, $file_index))) {
            ...
            for ($i = $file_index, $count = count($this->files->files_tokens[$file_name]); $i < $count - 1; $i++) {
                $add_index = 0;
                if ('[' === $this->files->files_tokens[$file_name][$i]) {// test to see if it is an array variable
                    ...
                    if ($this->is_property($file_name, $i - 1) ||
                            $this->is_method($file_name, $i - 1)) {// test to see if it is an object
        ...
        } elseif (($this->is_function($file_name, $file_index)) ||
                ($this->is_method($file_name, $file_index))) {
            return 'function';
        }
        return null;
    }
    ...
    function get_variable_index($variable_name, $function_name) {
        ...
        for ($i = $count - 1; $i >= 0; $i--) {
            if (( $variable_name === $this->parser_variables[$i]['variable_name'] ) &&
                    (0 === strcasecmp($this->parser_variables[$i]['function_name'], $function_name) )) {//note: PHP functions are not case sensitive
                return $i;
            }
        }
        return null;
    ...
    function set_vulnerable_variables() {
        for ($i = 0, $count = count($this->parser_variables); $i < $count; $i++) {
            ...
            if ((UNKNOWN != $variable['vulnerability_classification'] ) &&
                ...
                $parser_variables_with_index = array_merge(array('index' => $i), $this->parser_variables[$i]); //add $parser_variables index
